...
public abstract class BaseBlockDrawer extends BaseDrawer {
    ...
    @ColorInt
    protected int blockEnteredColor; // 已输入边框、填充、下划线颜色
    @BlockShape
    protected int blockShape;
    protected int blockLineWidth; //  正常边框、下划线宽度
    protected boolean isErrorState;
    protected boolean isEntereState;
    ...
    public BaseBlockDrawer(int blockNormalColor, int blockFocusColor, int blockErrorColor,
                           @BlockShape int blockShape, int blockLineWidth,
                           int blockCorner,boolean isEntereState) {
        this.blockNormalColor = blockNormalColor;
        this.blockFocusColor = blockFocusColor;
        this.blockErrorColor = blockErrorColor;
        this.blockEnteredColor = blockFocusColor;
        this.blockShape = blockShape;
        this.blockLineWidth = blockLineWidth;
        this.blockCorner = blockCorner;
        this.isEntereState = isEntereState;
        initPaint();
    }
    ...
    public final void drawCanvas() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i == currentBlockIndex && isFocused()) {
                ...
                if (isErrorState) {
                    ...
                    //当block为下划线时
                    if (currentBlockIndex>i && isEntereState){
                        drawEnteredBlock(rectF);
                    } else {
        ...
        //选中状态下block的颜色
        if (isEntereState && blockShape == BlockShape.UNDERLINE) {
            drawNormalBlock(fixPosition(currentBlockIndex));
        } else {
            if (isFocused()){
    ...
    // 已输入绘制
    protected abstract void drawEnteredBlock(RectF rectF);
    ...
    public int getBlockEnteredColor() {
        return blockEnteredColor;
    }

    public void setBlockEnteredColor(int blockEnteredColor) {
        this.blockEnteredColor = blockEnteredColor;
        drawCanvas();
    }
    ...
    public boolean isEntereState() {
        return isEntereState;
    }

    public void setEntereState(boolean entereState) {
        isEntereState = entereState;
        drawCanvas();
    }
}
